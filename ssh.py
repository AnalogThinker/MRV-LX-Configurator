...
CONFIRMED on-device (MRV LX-4048T-101AC):
  * `ssh -vv` -> LX offers host-key algos ssh-rsa, ssh-dss ONLY. Modern SSH
    disables these, so the default handshake failed "(no match)".
  * Login  : InReach / access -> lands directly in the CLI. Prompt "InReach:0 >".
  * Enable : `enable` + password `system` -> superuser prompt "InReach:0 >>".
...
# --- Algorithm sets (KEX matched at group-exchange-sha256; host-key was the
#     blocker). Broad legacy lists kept for safety. -------------------------
LEGACY_KEX = [
...
# CONFIRMED: LX offers only these two host-key algorithms.
...
class DeviceConfig:
    ...
    # Prompt regex. Matches user ">", superuser ">>", and config-mode labels
    # like "Config:0 >>", "Async1:0 >>", "Intf 1-1:0 >>". Rejects data lines
    # like "01: No entry".
...
class LXConnection:
    """Single long-lived SSH connection to one LX device."""
    ...
    def _connect_kwargs(self) -> dict:
        kw: dict = dict(
            ...
            known_hosts=None, connect_timeout=self.cfg.connect_timeout,
        )
    ...
    async def connect(self) -> None:
        if self._conn is not None:
            return
        log.info("Connecting to LX %s:%s as %s",
    ...
    async def _prepare(self, proc, force_enable: bool = False) -> None:
        """Drain login banner and optionally escalate to superuser."""
    ...
    async def run_config(self, commands: list[str], save: bool = False,
        ...
        """Apply config via the LX hierarchical modes:
           enable -> [configuration -> context] -> commands -> end -> [save]."""
        conn = await self._ensure()
        results: list[dict] = []
        async with self._lock:
            async with conn.create_process(term_type="vt100") as proc:
    ...
    # -- introspection (context-sensitive "?" help) -------------------------

    async def cli_help(self, tokens: str = "", context: Optional[str] = None) -> dict:
        """Ask the device what it supports (side-effect free): type "<tokens> ?"
        WITHOUT Enter, read help, Ctrl-U to clear the line, then leave config."""
        conn = await self._ensure()
        async with self._lock:
            async with conn.create_process(term_type="vt100") as proc:
                await self._prepare(proc, force_enable=True)
    ...
    async def _read_until_prompt(self, proc, timeout: int) -> str:
        ...
        try:
            ...
            log.debug("read_until_prompt timed out")
...
def _find_error(text: str) -> Optional[str]:
    """Return the first device-error signature found in output, else None."""
    m = _ERROR_RE.search(text or "")
    return m.group(0) if m else None


def _clean(text: str, command: str, prompt: "re.Pattern", pager: "re.Pattern") -> str:
    """Strip ANSI escapes, normalise newlines, drop the echoed command, prompt
    lines and pager banners so the UI shows just the command's output."""
